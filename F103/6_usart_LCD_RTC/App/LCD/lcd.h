...
//#define LCD_RST_Pin                SYS_Pinx
//#define LCD_RST_GPIO_CLK_ENABLE()       do{ __HAL_RCC_GPIOx_CLK_ENABLE(); }while(0)   /* 所在IO口时钟使能 */



#define LCD_DATA_GPIO_Port              GPIOB
#define LCD_DATA_Pin               GPIO_PIN_All                                /* 16个IO都用到 */
#define LCD_DATA_GPIO_CLK_ENABLE()      do{ __HAL_RCC_GPIOB_CLK_ENABLE(); }while(0)   /* 所在IO口时钟使能 */

/* LCD 端口控制函数定义 */
/* 利用操作寄存器的方式控制IO引脚提高屏幕的刷新率 */
#define LCD_BL(x)       LCD_BL_GPIO_Port->BSRR = LCD_BL_Pin << (16 * (!x))     /* 设置BL引脚 */

#define LCD_WR(x)       LCD_WR_GPIO_Port->BSRR = LCD_WR_Pin << (16 * (!x))     /* 设置WR引脚 */
#define LCD_RD(x)       LCD_RD_GPIO_Port->BSRR = LCD_RD_Pin << (16 * (!x))     /* 设置RD引脚 */
#define LCD_CS(x)       LCD_CS_GPIO_Port->BSRR = LCD_CS_Pin << (16 * (!x))     /* 设置CS引脚 */
#define LCD_RS(x)       LCD_RS_GPIO_Port->BSRR = LCD_RS_Pin << (16 * (!x))     /* 设置RS引脚 */
...
/* 以下是使用HAL库方式控制IO引脚 */
//#define LCD_RST(x)      sys_Pin_set(LCD_RST_GPIO_Port,  LCD_RST_Pin,  x)  /* 设置RST引脚 */

/* LCD背光控制 */
//#define LCD_BL(x)   do{ x ? \
//                      HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin, Pin_SET) : \
//                      HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin, Pin_RESET); \
//                     }while(0)

//#define LCD_WR(x)   do{ x ? \
//                      HAL_GPIO_WritePin(LCD_WR_GPIO_Port, LCD_WR_Pin, Pin_SET) : \
//                      HAL_GPIO_WritePin(LCD_WR_GPIO_Port, LCD_WR_Pin, Pin_RESET); \
//                     }while(0)     /* 设置WR引脚 */
//#define LCD_RD(x)   do{ x ? \
//                      HAL_GPIO_WritePin(LCD_RD_GPIO_Port, LCD_RD_Pin, Pin_SET) : \
//                      HAL_GPIO_WritePin(LCD_RD_GPIO_Port, LCD_RD_Pin, Pin_RESET); \
//                     }while(0)     /* 设置RD引脚 */
//#define LCD_CS(x)   do{ x ? \
//                      HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, Pin_SET) : \
//                      HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, Pin_RESET); \
//                     }while(0)     /* 设置CS引脚 */
//#define LCD_RS(x)   do{ x ? \
//                      HAL_GPIO_WritePin(LCD_RS_GPIO_Port, LCD_RS_Pin, Pin_SET) : \
//                      HAL_GPIO_WritePin(LCD_RS_GPIO_Port, LCD_RS_Pin, Pin_RESET); \
//                     }while(0)     /* 设置RS引脚 */
